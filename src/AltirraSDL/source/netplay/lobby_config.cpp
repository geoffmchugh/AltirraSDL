...
const char* const kDefaultLobbyIni =
	...
	"enabled = true\n"
	"\n"
	"; Backup lobby — same server, alternate DNS via DuckDNS. It is disabled\n"
	"; by default on every platform: enabled HTTP entries are federated, not\n"
	"; ordered failovers, so enabling both aliases would duplicate requests\n"
	"; to the same physical service. Enable it only when the primary name is\n"
	"; unavailable. For the WASM (browser) build it should normally remain\n"
	"; disabled because DuckDNS is commonly blocked by privacy extensions.\n"
	"; DuckDNS is on most ad-blocker / privacy-extension blocklists\n"
	"; (uBlock Origin's EasyPrivacy, AdGuard's tracking lists, etc. flag\n"
	"; *.duckdns.org as a dynamic-DNS service heavily abused for C2).  A\n"
	"; WASM joiner that happens to pick the duckdns copy of a session out\n"
	"; of Browse would be unable to open the WebSocket — and same-origin\n"
	"; doesn't help, since the lobby URL is what the WS connects to.  If\n"
	"; the primary atari.org.pl DNS were unreachable, the WASM bundle\n"
	"; couldn't be loaded either, so the WASM-side DuckDNS backup is moot.\n"
	"; Trade-off when enabled: while BOTH names resolve to our server,\n"
	"; hosting a game registers it on each lobby separately — the same\n"
	"; physical session appears twice in Browse (different sessionIds,\n"
	"; same backend).  Flip this to `enabled = true` to opt back in.\n"
	"[backup]\n"
