		...
		++it)
	{
		if (it->mpDevice == dev) {
			if (it->mbChild) {
				...
				if (parent)
					parent->GetDeviceBusById(dev->GetParentBusId())->RemoveChildDevice(dev);
			}
...
void ATDeviceManager::AppendPathForDevice(VDStringA& s, IATDevice *dev, bool recurse) const {
	...
	if (parent) {
		...
		const auto busId = dev->GetParentBusId();
...
void ATDeviceManager::ReconfigureDevice(IATDevice& dev, const ATPropertySet& pset) {
	...
	if (parent) {
		busId = dev.GetParentBusId();
		bus = parent->GetDeviceBusById(busId);
		if (bus)
			bus->RemoveChildDevice(&dev);
		else
			VDFAIL("Device references nonexistent parent bus");
	}
...
void ATDeviceManager::Mark(IATDevice *dev, IATDevice *const *pExcludedDevs, size_t numExcludedDevs, vdhashset<IATDevice *>& devSet) {
	...
	for(uint32 busIndex = 0; ; ++busIndex) {
		sint32 busId = parent->GetDeviceBusIdByIndex(busIndex);
		if (busId < 0)
			break;
...
void ATDeviceManager::SerializeDevice(IATDevice *dev, VDJSONWriter& out, bool includeChildren) const {
	if (dev) {
		...
		if (dp && includeChildren) {
			...
			for(uint32 busIndex = 0; ; ++busIndex) {
				sint32 busId = dp->GetDeviceBusIdByIndex(busIndex);
				if (busId < 0)
					break;

				IATDeviceBus *bus = dp->GetDeviceBusById(busId);
				if (!bus) {
					VDFAIL("Device returned invalid bus ID");
					continue;
				}
...
void ATDeviceManager::DeserializeDevice(IATDeviceParent *parent, IATDeviceBus *bus, const VDJSONValueRef& node) {
	...
	if (bus) {
		...
		for(uint32 busIndex = 0; ; ++busIndex) {
			...
			IATDeviceBus *tryBus = parent->GetDeviceBusById(busId);
			if (!tryBus) {
				VDFAIL("Device returned invalid bus ID");
				continue;
			}
	...
	if (devParent) {
		...
		if (buses.IsObject()) {
			for(const auto& busEntry : buses.AsObject()) {
				...
				if (1 == swscanf(busIndexStr, L"%u%lc", &busId, &dummy)) {
					IATDeviceBus *deviceBus = devParent->GetDeviceBusById(busId);
