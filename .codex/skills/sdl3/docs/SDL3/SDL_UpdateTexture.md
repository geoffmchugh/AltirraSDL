...
area to update, or NULL to update the entire texture. |
| const void * | pixels | the raw pixel data in the format of the texture. |
| int | pitch | the number of bytes in a row of pixel data, including padding
between lines. |
...
The pixel data must be in the pixel format of the texture, which can
be queried using the [SDL_PROP_TEXTURE_FORMAT_NUMBER](SDL_PROP_TEXTURE_FORMAT_NUMBER)
property.
