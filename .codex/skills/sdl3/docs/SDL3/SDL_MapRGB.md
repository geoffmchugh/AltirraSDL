...
| constSDL_PixelFormatDetails* | format | a pointer toSDL_PixelFormatDetailsdescribing
the pixel format. |
| --- | --- | --- |
| constSDL_Palette* | palette | an optional palette for indexed formats, may be NULL. |
...
and returns the pixel value best approximating the given RGB color value
...
a 16-bpp format the return value can be assigned to a [Uint16](Uint16), and similarly a Uint8 for an 8-bpp
format).
