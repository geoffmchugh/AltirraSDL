...
Draws data using bound graphics state and with draw parameters set
from a buffer.
...
| Uint32 | draw_count | the number of draw parameter sets that should be read from the draw
buffer. |

## Remarks

The buffer must consist of tightly-packed draw parameter sets that
each match the layout of [SDL_GPUIndirectDrawCommand](SDL_GPUIndirectDrawCommand).
