...
| SDL_GPUBuffer*const * | storage_buffers | an array of storage buffer binding structs. |
| Uint32 | num_bindings | the number of storage buffers to bind from the array. |

## Remarks

These buffers must have been created with [SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ](SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ).
