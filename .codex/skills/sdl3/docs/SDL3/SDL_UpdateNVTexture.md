...
Update a rectangle within a planar NV12 or NV21 texture with new
pixels.
...
| --- | --- | --- |
| constSDL_Rect* | rect | a pointer to the rectangle of pixels to update, or NULL to update
the entire texture. |
...
| int | UVpitch | the number of bytes between rows of pixel data for the UV
plane. |
...
You can use [SDL_UpdateTexture](SDL_UpdateTexture)()
as long as your pixel data is a contiguous block of NV12/21 planes in
the proper order, but this function is available if your pixel data is
not contiguous.
