...
| SDL_Window* | window | the window that should become the child of a parent. |
| --- | --- | --- |
| SDL_Window* | parent | the new parent window for the child window. |
...
modal state will fail. Use [SDL_SetWindowModal](SDL_SetWindowModal)() to cancel the
modal status before attempting to change the parent.

Popup windows cannot change parents and attempts to do so will
fail.
