...
| SDL_AsyncIOQueue* | queue | the async I/O task queue to signal. |
| --- | --- | --- |

## Remarks

This will unblock any threads that are sleeping in a call to [SDL_WaitAsyncIOResult](SDL_WaitAsyncIOResult) for the
specified queue, and cause them to return from that function.
...
using [SDL_WaitThread](SDL_WaitThread)() to make sure
they've done so).
