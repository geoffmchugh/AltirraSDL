...
be cleaned out still, too. Read this page with some skepticism for
now.
...
- Install minimal Java environment. For instance, in
Debian/Ubuntu:
...
- Install NDK (tested with [[https://dl.google.com/android/repository/android-ndk-r10e-linux-x86_64.zip](https://dl.google.com/android/repository/android-ndk-r10e-linux-x86_64.zip)
r10e])

- Install the latest SDK, run `tools/bin/sdkmanager` (or
`tools/android` pre-2017) and install one API (>= 31)
...
- use OpenJDK 17: execute
`sudo update-alternatives --config java` and select jdk-17 as
default; or use
...
### SDL wrapper + SDL_image NDK
module

Let's modify `SDL3_image/showimage.c` to show a simple
embedded image (e.g. XPM).
...
## Build an
autotools-friendly environment

You use autotools in your project and can't be bothering
understanding ndk-build's cryptic errors? This guide is for you!

Note: this environment can be used for CMake too.

### Compile a
shared binaries bundle for SDL and SDL_*
...
- Optionnaly edit `jni/Android.mk` to disable some formats,
e.g.:
...
Note: no need to add `System.loadLibrary` calls in
`SDLActivity.java`, your application will be linked to them
and Android's ld-linux loads them automatically.

### Install SDL in a GCC
toolchain
...
- Install `pkg-config` and install a host-triplet-prefixed
symlink in the PATH (auto-detected by autoconf):
...
Static builds (`--disable-shared`) are recommended for
simplicity (no additional `.so` to declare).
...
- Copy and adapt the `/usr/src/SDL3/android-project`
skeleton as explained in `README-android.md`. You can leave
it as-is in a first step.
...
- Add `/usr/src/SDL3/src/main/android/SDL_android_main.c`
in your project (comment out the line referencing "SDL_internal.h").
...
- In your `Makefile.am`, tell Automake you'll build
executables as libraries, using something like:
...
how to support `armeabi-v7a` and document what devices
...
You can use our Android GCC toolchain using a simple toolchain
file:
...
`Failure [INSTALL_FAILED_INSUFFICIENT_STORAGE]`, even if you
have free local storage, that may mean anything. Check logcat first:
...
this probably means your `SDLActivity.java` is out-of-sync
with your libSDL3.so.
