...
| float | refresh_rate | the refresh rate of the desired display mode, or 0.0f for the
desktop refresh rate. |
| bool | include_high_density_modes | boolean to include high density modes in the search. |
| SDL_DisplayMode* | closest | a pointer filled in with the closest display mode equal to or larger
...
rate. If all the available modes are too small, then false is
returned.
