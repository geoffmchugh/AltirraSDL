...
For many of the most common functions, like [SDL_memcpy](SDL_memcpy), SDL might just call through to
the usual C runtime behind the scenes, if it makes sense to do so (if
it's faster and always available/reliable on a given platform), reducing
...
like [SDL_crc32](SDL_crc32)() and [SDL_reinterpret_cast](SDL_reinterpret_cast), etc. It also
offers a few better options, like [SDL_strlcpy](SDL_strlcpy)(), which functions as a safer
form of strcpy().
