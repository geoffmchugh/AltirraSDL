...
| Sint64 | wait_semaphore | a VkSempahore to wait on before rendering the current frame, or 0 if
not needed. |
| Sint64 | signal_semaphore | a VkSempahore that SDL will signal when rendering for the current
frame is complete, or 0 if not needed. |
...
The Vulkan renderer will wait for `wait_semaphore` before
submitting rendering commands and signal `signal_semaphore`
...
synchronization. Querying [SDL_PROP_RENDERER_VULKAN_SWAPCHAIN_IMAGE_COUNT_NUMBER](SDL_PROP_RENDERER_VULKAN_SWAPCHAIN_IMAGE_COUNT_NUMBER)
will give you the maximum number of semaphores you'll need.

## Thread Safety

It is **NOT** safe to call this function from two
threads at once.
