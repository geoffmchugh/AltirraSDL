...
| --- | --- | --- |
| SDL_Texture* | texture | the targeted texture, which must be created with theSDL_TEXTUREACCESS_TARGETflag, or NULL to render to the window instead of a texture. |
...
Viewport, cliprect, scale, and logical presentation are unique to
each render target. Get and set functions for these states apply to the
