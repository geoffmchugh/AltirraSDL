...
| Uint64* | state | a pointer to the current random number state, this may not be
NULL. |
| --- | --- | --- |
...
([Sint32](Sint32)) Returns a random value in the
range of [0 .. n-1].

## Remarks

The method used is faster and of better quality than
`rand() % n`. Odds are roughly 99.9% even for n = 1 million.
...
[Sint32](Sint32), you should use: ([Sint32](Sint32))[SDL_rand_bits_r](SDL_rand_bits_r)(state)
...
This function is thread-safe, as long as the state pointer isn't
