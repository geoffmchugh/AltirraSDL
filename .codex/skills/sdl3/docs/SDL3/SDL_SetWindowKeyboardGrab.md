...
| SDL_Window* | window | the window for which the keyboard grab mode should be set. |
| --- | --- | --- |
...
shortcuts can be captured by applications (one example is Ctrl+Alt+Del
on Windows).
...
when the window is full-screen to ensure the user is not trapped in your
application. If you have a custom keyboard shortcut to exit fullscreen
mode, you may suppress this behavior with [SDL_HINT_ALLOW_ALT_TAB_WHILE_GRABBED](SDL_HINT_ALLOW_ALT_TAB_WHILE_GRABBED).
