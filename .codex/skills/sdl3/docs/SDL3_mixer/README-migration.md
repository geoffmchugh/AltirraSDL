...
That being said, we think SDL3_mixer and SDL3 are both great pieces
of software--significant improvements over their previous versions--and
we think that once you move to them, you'll be quite happy you did.

There are a lot of things that don't have simple replacements that
...
equivalent of Mix_GetNumTracks(), because messing with the specifics of
MOD files in the public API is both uncommon and generally pretty
messy.

This migration guide will attempt to walk through the important
details but it's possible that some things can't be done the same way.
Feel free to open bug reports if you're totally stuck.
...
other file formats, just not to native midi hardware). We have ripped
this piece out of SDL2_mixer and packaged it as a separate library that
can be used alongside SDL3_mixer, or without SDL_mixer at all: https://github.com/libsdl-org/SDL_native_midi
...
In SDL2_mixer, all functions started with `Mix_` and
macros started with `MIX_`. In SDL3_mixer, everything starts
with `MIX_`.
...
Previously you had to tell SDL_mixer what file formats you expected
to use during Mix_Init(). Now you don't:
...
MIX_CreateMixerDevice() accepts an SDL_AudioSpec as a hint, but makes
...
SDL2_mixer expected you to lock the SDL audio device directly if you
needed explicit synchronization. SDL3 doesn't have a direct equivalent
of SDL2's SDL_LockAudioDevice(), and SDL3_mixer might be operating
without an audio device at all, so there are now explicit
MIX_LockMixer() and MIX_UnlockMixer() API calls available.
...
Preloaded (but not necessarily predecoded) audio is managed in
...
A special function in SDL2_mixer, Mix_QuickLoad_RAW(), has been
...
without any extra allocations or copies.
...
SDL2_mixer had "channels," each of which contained a single chunk to
...
SDL3_mixer has "tracks," each containing a single input (a loaded
...
for: `MIX_PROP_METADATA_TITLE_STRING`. This allows SDL3_mixer
to expose non-standard information, but also offer easy lookup of the
most common, and most important, metadata.

After loading an audio file, MIX_GetAudioProperties() will provide
the metadata (and also provide a convenient place for the app to hang
their own app-specific data, as well).
...
SDL2_mixer had several "effects" that were internal code that used
...
Tag tracks with MIX_TagTrack() and remove a tag with
MIX_UntagTrack().

## Playing

There are several different functions in SDL2_mixer to start a
...
MIX_StopTag(), the fade out is specified in *milliseconds*, since
we can't guarantee that all tracks have the same sample rate.

## Pausing

Mix_Pause(), Mix_PauseMusic(), Mix_Resume() and Mix_ResumeMusic()
became MIX_PauseTrack() and MIX_ResumeTrack().
...
destroy *everything* that SDL3_mixer created, including the
mixers and MIX_Audio objects, which might be more convenient.

## tl;dr

A very brief comment on what to do with each symbol in
SDL2_mixer.
...
- Mix_QuickLoad_WAV => MIX_LoadAudio_IO with an
SDL_IOFromConstMem() and closeio=true.
...
- Mix_GetMusicTitleTag => MIX_PROP_METADATA_TITLE_STRING property
with Mix_GetAudioProperties()

- Mix_GetMusicArtistTag => MIX_PROP_METADATA_ARTIST_STRING property
with Mix_GetAudioProperties()

- Mix_GetMusicAlbumTag => MIX_PROP_METADATA_ALBUM_STRING property
with Mix_GetAudioProperties()
...
- Mix_HookMusic => MIX_SetTrackAudioStream and set a get-callback
on that stream, start track playing.
...
- Mix_UnregisterAllEffects => MIX_SetTrackCookedCallback, plus
optional MIX_SetTrackStoppedCallback

- MIX_EFFECTSMAXSPEED => no equivalent in SDL3_mixer.

- Mix_SetPanning => MIX_SetTrackStereo or
...
- Mix_FadeInChannelTimed => MIX_SetTrackAudio then
MIX_PlayTrack

- Mix_Volume => MIX_SetTrackGain

- Mix_VolumeChunk => MIX_SetTrackGain; gain is not set on
MIX_Audio, only on MIX_Track
...
- Mix_SetTimidityCfg => FIXME currently
SDL_setenv("TIMIDITY_CFG")
