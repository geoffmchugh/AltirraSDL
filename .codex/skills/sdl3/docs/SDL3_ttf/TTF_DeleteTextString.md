...
| --- | --- | --- |
| int | offset | the offset, in bytes, from the beginning of the string if >= 0,
the offset from the end of the string if < 0. Note that this does not
do UTF-8 validation, so you should only delete at UTF-8 sequence
boundaries. |
| int | length | the length of text to delete, in bytes, or -1 for the remainder of
the string. |
