...
| --- | --- | --- |
| int | x | the x coordinate in pixels, positive from the left edge towards the
right. |
| int | y | the y coordinate in pixels, positive from the top edge towards the
bottom. |
...
`text` must have been created using a [TTF_TextEngine](TTF_TextEngine) from [TTF_CreateSurfaceTextEngine](TTF_CreateSurfaceTextEngine)().
