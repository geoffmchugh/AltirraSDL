...
| IMG_AnimationStream* | stream | the stream receiving images. |
| --- | --- | --- |
| SDL_Surface * | surface | the surface to add as the next frame in the animation. |
| Uint64 | pts | the presentation timestamp of the frame, usually in milliseconds but
can be other units if theIMG_PROP_ANIMATION_STREAM_CREATE_TIMEBASE_DENOMINATOR_NUMBERproperty is set when creating the stream. |
